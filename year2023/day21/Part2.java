...
public class Part2 {
    ...
    public Part2() throws FileNotFoundException {
        ...
        sum += numberOfPlotsInOneWay
                * calculateNumberOfSteps(lines, lines.size() - 1, lines.size() - 1, lines.size() / 2 - 1);
        sum += (numberOfPlotsInOneWay - 1) * calculateNumberOfSteps(lines, 0, 0, lines.size() * 3 / 2 - 1);
        sum += (numberOfPlotsInOneWay - 1)
                * calculateNumberOfSteps(lines, 0, lines.size() - 1, lines.size() * 3 / 2 - 1);
        sum += (numberOfPlotsInOneWay - 1)
                * calculateNumberOfSteps(lines, lines.size() - 1, 0, lines.size() * 3 / 2 - 1);
        sum += (numberOfPlotsInOneWay - 1)
                * calculateNumberOfSteps(lines, lines.size() - 1, lines.size() - 1, lines.size() * 3 / 2 - 1);
    ...
    private static void takeNextStep(ArrayList<String> lines, Set<Position> positions) {
        ...
        while (iterator.hasNext()) {
            ...
            if (position.getY() < lines.get(0).length() - 1
                    && lines.get(position.getX()).charAt(position.getY() + 1) != '#') {
                newPositions.add(new Position(position.getX(), position.getY() + 1));
            }
        }
