...
public class Part2 {
    ...
    private static ArrayList<String> analyzePipes(ArrayList<String> pipes) {
        ...
        while (pipes.get(next[0]).charAt(next[1]) != 'S') {
            ...
            String modifiedString = originalString.substring(0, next[1]) + newChar
                    + originalString.substring(next[1] + 1);
        ...
        for (int i = 0; i < rightDirections.get(0).length; i++) {
            for (int j = 0; j < rightDirections.get(1).length; j++) {
                if (rightDirections.get(0)[i] == rightDirections.get(1)[j]) {
                    ...
                    String modifiedString = originalString.substring(0, start[1]) + newChar
                            + originalString.substring(start[1] + 1);
    ...
    private static boolean checkDirection(char[] direction, int[] start, List<String> pipes, int[] next, int rowChange,
            int colChange) {
        for (int i = 0; i < direction.length; i++) {
            int newRow = start[0] + rowChange;
            int newCol = start[1] + colChange;

            if (newRow > -1 && newRow < pipes.size() && newCol > -1 && newCol < pipes.get(0).length()
                    && pipes.get(newRow).charAt(newCol) == direction[i]) {
                next[0] = newRow;
                next[1] = newCol;
                return false;
            }
