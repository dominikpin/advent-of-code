...
public class Part1 {
    ...
    private static int dykstrasAlgorithm(int[][] map) {
        ...
        while (queue.size() > 0) {
            ...
            boolean found = false;
            for (int[] element1 : seen.reversed()) {
                boolean isSame = true;
            ...
            if (element[5] < 3 && !(element[3] == 0 && element[4] == 0)) {
                ...
                if (nextRow >= 0 && nextRow < map.length && nextColumn >= 0 && nextColumn < map[0].length) {
                    addToQueue(queue, element[0] + map[nextRow][nextColumn], nextRow, nextColumn, element[3],
                            element[4], element[5] + 1);
                }
            }
            for (int i = 0; i < DIRECTIONS.length; i++) {
                if ((DIRECTIONS[i][0] == element[3] && DIRECTIONS[i][1] == element[4])
                        || (DIRECTIONS[i][0] == -element[3] && DIRECTIONS[i][1] == -element[4])) {
                    continue;
                }
                int nextRow = element[1] + DIRECTIONS[i][0];
                int nextColumn = element[2] + DIRECTIONS[i][1];
                if (nextRow >= 0 && nextRow < map.length && nextColumn >= 0 && nextColumn < map[0].length) {
                    addToQueue(queue, element[0] + map[nextRow][nextColumn], nextRow, nextColumn, DIRECTIONS[i][0],
                            DIRECTIONS[i][1], 1);
                }
            }
        }
