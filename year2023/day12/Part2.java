...
public class Part2 {
    ...
    private static long analyzeString(String line) {
        ...
        long sum = calculateNumberOfWays(springsLine, springNumbers, 0, 0, 0);
        memoTable.clear();
        return sum;
    }
    ...
    private static long calculateNumberOfWays(String springsLine, int[] numbers, int i, int numbersI,
            int currentBroken) {
        ...
        if (i == springsLine.length()) {
            if (numbersI == numbers.length && currentBroken == 0
                    || numbersI == numbers.length - 1 && currentBroken != 0 && numbers[numbersI] == currentBroken) {
        ...
        if (springsLine.charAt(i) != '.') {
            sum += calculateNumberOfWays(springsLine, numbers, i + 1, numbersI, currentBroken + 1);
        }
        if (springsLine.charAt(i) != '#') {
            if (currentBroken != 0) {
                if (numbersI < numbers.length && currentBroken == numbers[numbersI]) {
                    sum += calculateNumberOfWays(springsLine, numbers, i + 1, numbersI + 1, 0);
                }
            } else {
                sum += calculateNumberOfWays(springsLine, numbers, i + 1, numbersI, 0);
            }
        }
