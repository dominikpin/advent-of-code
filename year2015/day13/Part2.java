...
public class Part2 {
    ...
    private static int calculateHappiness(ArrayList<Person> table) {
        ...
        for (int i = 0; i < table.size(); i++) {
            ...
            Person personLeft = i != 0 ? table.get(i - 1) : null;
            Person personRight = i != table.size() - 1 ? table.get(i + 1) : null;
            sum += (personLeft != null ? personMain.getHappiness(personLeft) : 0) +
                    (personRight != null ? personMain.getHappiness(personRight) : 0);
        }
        return sum;
    }
}